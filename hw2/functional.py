...
def relu_backward(y, x):
  ...
  x.grad += y.grad*(x>0).float().to(device)
  # END SOLUTION
...
def softmax(x, ctx=None):
  ...
  # BEGIN SOLUTION
  # x = x.to(device)
  y = torch.exp(x-torch.logsumexp(x,1, keepdim=True).to(device))
  if ctx is not None:
...
def softmax_backward(y, x):
  ...
  # print(J.shape)
  # print(y.grad.shape)
  res = J*y.grad.reshape(y.grad.shape[0],y.grad.shape[1],1).to(device)
...
def cross_entropy_backward(loss, pred, target):
  ...
  eps = torch.finfo(pred.dtype).tiny
  # BEGIN SOLUTION
  res = (-1/(pred+eps)).to(device) * one_hot(target,num_classes=pred.shape[1])
